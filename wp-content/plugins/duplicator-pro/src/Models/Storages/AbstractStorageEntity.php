...
abstract class AbstractStorageEntity extends DUP_PRO_Storage_Entity implements AbstractStorageEntityInterface, ModelMigrateSettingsInterface
{
    ...
    public function isFull()
    {
        ...
        return (count($packagesList) >= $this->config['max_packages']);
    }
    ...
    public function purgeOldPackages($keepList = [])
    {
        ...
        try {
            ...
            $numToDelete = count($packagesList) - $this->config['max_packages'];
            for ($i = 0; $i < $numToDelete; $i++) {
                if (in_array($packagesList[$i], $keepList)) {
                    DUP_PRO_Log::trace("Skipping old Backup purging of " . $packagesList[$i]);
                    $numToDelete++;
                    if ($numToDelete > count($packagesList)) {
                        break;
                    }
                ...
                DUP_PRO_Log::trace("Purging Backup " . $packName  . " at " . $this->name . '[ID: ' . $this->id . '] type: ' . $this->getSTypeName());
