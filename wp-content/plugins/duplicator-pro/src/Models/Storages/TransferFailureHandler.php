...
{
    const OPTION_KEY_FAILED_TRANSFERS = 'duplicator_failed_transfers';
    ...
    {
        add_action('duplicator_pro_transfer_failed', [self::class, 'addFailedTransfer'], 10, 1);
    ...
    protected static function getTranferFailedMessage(array $storageInfos, bool $isAllDownload, bool $isAllUpload): string
    {
        if (count($storageInfos) === 1) {
            ...
            $message = sprintf(wp_kses($message, [ 'b' => [], 'i' => [] ]), $storageInfos[0]['name'], $storageInfos[0]['type']);
        } else {
     ...
     * @return void
     */
    public static function addFailedTransfer(DUP_PRO_Package_Upload_Info $uploadInfo): void
    {
