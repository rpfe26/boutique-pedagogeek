...
class FDAP_Shortcodes {
    ...
    public function enqueue_styles() {
        if (is_page('fdap-2') || is_page('mes-fdap') || is_singular('fdap')) {
            ...
            wp_enqueue_style('fdap-selector-style', FDAP_PLUGIN_URL . 'assets/css/competence-selector.css', [], FDAP_VERSION);
            
            wp_enqueue_script('fdap-selector-js', FDAP_PLUGIN_URL . 'assets/js/competence-selector.js', [], FDAP_VERSION, true);
            
            // Audio recorder pour saisie élève + preview fichiers + prof
    ...
    public function form_shortcode($atts) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fdap_nonce'])) {
            if (wp_verify_nonce($_POST['fdap_nonce'], 'fdap_form_submit')) {
                ...
                if ($result) {
                    wp_redirect(add_query_arg(['msg' => 'saved'], get_permalink()));
                    exit;
                }
            }
        }
        ...
        ob_start();
        echo '<div class="fdap-main-wrapper fdap-form-container">';
                ...
                <div class="fdap-table-wrapper">
                    <table class="fdap-table">
                        <thead>
                            <tr>
                                <?php if ($is_admin): ?>
                                    <th><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="margin-right: 6px;"><path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"/></svg> Élève</th>
                                <?php endif; ?>
                                <th><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="margin-right: 6px;"><path d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z"/><path d="M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5h3zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3z"/></svg> Activité</th>
                                <th><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="margin-right: 6px;"><path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5zM1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4H1z"/></svg> Date</th>
                                <th><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="margin-right: 6px;"><path d="M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10zm0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6z"/></svg> Lieu</th>
                                <th><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="margin-right: 6px;"><path d="M2.5 8a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0 6a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5z"/></svg> Statut</th>
                                <th>Actions</th>
                            </tr>
    ...
    private function save_fiche() {
        ...
        if ($action === 'update' && isset($_POST['fdap_id'])) {
            ...
            // Si un élève modifie sa fiche, elle repasse en "publié" pour contrôle
            if (!$is_admin) {
        ...
        if ($is_admin && (isset($_POST["fdap_comment_text"]) || isset($_POST["fdap_comment_audio_data"]))) {
            ...
            if (!empty($comment_text) || !empty($comment_audio_data)) {
                ...
                wp_redirect(add_query_arg(["msg" => "saved", "fdap_id" => $post_id], get_permalink(get_page_by_path("fdap-2"))));
                exit;
            }
